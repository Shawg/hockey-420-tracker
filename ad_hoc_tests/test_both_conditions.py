#!/usr/bin/env python3
"""
Test detection of both elapsed and remaining 4:20 goals on Oct 19, 2025.
"""
...
for game in games:
    ...
    for goal in goals_420:
        # Get actual player names (detector only returns Player #ID)
        matching_plays = []
        if goal["condition"] == "elapsed":
            matching_plays = [
                p for p in plays 
                if p.get("typeDescKey") == "goal" and p.get("timeInPeriod") == goal["time"].replace(" remaining", "")
            ]
        else:  # remaining
            matching_plays = [
                p for p in plays 
                if p.get("typeDescKey") == "goal" and p.get("timeRemaining") == goal["time"].replace(" remaining", "")
            ]
